...
exports.list = asyncHandler(async (req, res) => {
  ...
  const items = await Customer.find(filter).sort({ name: 1 });
  res.json({ items });
});
...
exports.ledger = asyncHandler(async (req, res) => {
  const entries = await LedgerEntry.find({ party: req.params.id, partyModel: 'Customer' }).sort({ date: 1, createdAt: 1 });
  res.json({ entries });
});

exports.outstanding = asyncHandler(async (req, res) => {
  const items = await Customer.find({ currentBalancePaisa: { $gt: 0 } }).sort({ currentBalancePaisa: -1 });
  res.json({ items });
});
